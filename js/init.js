...
function arlo_tm_popup_news(){
	...
	var list		= jQuery('.special_skill .skill_inner ul li');
	var popupMenu	= jQuery('.arlo_tm_popup_news');
	var popupInner	= popupMenu.find('.skill_inner');
...
function arlo_tm_down(){
	...
	jQuery('.top_wrap a').on('click',function(){
		if($.attr(this, 'href') !== '#'){
			$('html, body').animate({
...
function arlo_tm_kenburn_slider(){
	...
	"use strict";
		...
		jQuery(function() {
			jQuery('.yuki_header .overlay_slider').vegas({
			timer:false,	
			animation: [ 'kenburnsUp',  'kenburnsLeft', 'kenburnsRight'],
			delay:7000,

			slides: [
				{ src: 'image/home/top_wrap3.jpg' },
				{ src: 'image/home/top_wrap7.jpg' },
				{ src: 'image/home/top_wrap8.jpg' },
			]
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.yuki_animation_text_word1');
	
		animateSpan.typed({
			strings: [' " Because I have a dream, a life is brilliant." '],
			loop: false,
			startDelay: 3e3,
			backDelay: 2e3,
			// delay:5000
	...
	var animateSpan			= jQuery('.yuki_animation_text_word2');
...
function yuki_favorite(){
	...
	if(jQuery().isotope) {
		...
		// Needed variables
		var list 		 = jQuery('.yuki_favorite .favorite_list ul');
		var filter		 = jQuery('.yuki_favorite .portfolio_filter ul');
...
function arlo_tm_projects() {
	...
	jQuery('.yuki_favorite_animation_wrap').each(function() {
		jQuery(this).on('mouseenter', function() {
			if (jQuery(this).data('title')) {
				jQuery('.yuki_favorite_titles').html(jQuery(this).data('title') + '<span class="work__cat">' + jQuery(this).data('category') + '</span>');
				jQuery('.yuki_favorite_titles').addClass('visible');
			}

			jQuery(document).on('mousemove', function(e) {
				jQuery('.yuki_favorite_titles').css({
					left: e.clientX - 10,
					top: e.clientY + 25
				});
			});
		}).on('mouseleave', function() {
			jQuery('.yuki_favorite_titles').removeClass('visible');
		});
	});
}
...
function arlo_tm_totop(){
	...
	jQuery(".yuki_footer .footer_inner .top a").on('click', function(e) {
		e.preventDefault();		
